...
from data import normalize, to_uniform
...
def get_layer_stereo_masks(timesteps: int, pass_size: int):
    """
    Generate L and R stereo masks of size (timesteps, 1), with
    linear variation across the layer dimension. To be multiplied
    with audio tensor.
    """
    left_mask = torch.arange(1, 0, -1/pass_size)
    right_mask = torch.ones(pass_size) - left_mask

    n_passes = int(timesteps / pass_size)
    left_mask = left_mask.repeat((n_passes,)).unsqueeze(1)
    right_mask = right_mask.repeat((n_passes,)).unsqueeze(1)
...
def sonify(history: torch.Tensor, note_length, fs=44100, do_stereo=True, do_interpolate=False, do_diff=False):
    ...
    """
    # timer = Timer()
    history = to_uniform(history, 100, 5000)
...
def gainson(gains: torch.Tensor, freq_map: torch.Tensor, note_length, fs=44100, do_stereo=True):
    ...
    T, V = gains.shape
    gains = normalize(gains, lower=0, upper=0.5)
    samples_per_note = int(fs * note_length)
